...
-Ensure that all required dependencies are installed:
    ...
    nodemon
    concurrently
-To get the web application up and running, app.js must be executed (found at src > web > app.js). 
    you can use the command 'npx nodemon app.js' from this folder. Or alternatively use the command 'npm start' from the root, which will start both the app and the server.
-When reviewing the API that has been built a script has been created which should enable both the app.js and the server.js to run at the same time. 
    Please use "npm start" to ensure both are running. 
